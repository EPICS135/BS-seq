...
$mC_sequenced_in_chrC1=0;$C_sequenced_in_chrC1=0.1; 
$mC_sequenced_in_chrC2=0;$C_sequenced_in_chrC2=0.1;
$mC_sequenced_in_chrC3=0;$C_sequenced_in_chrC3=0.1;
while(my $line=<F1>){
	chomp $line;
	my @a=split/\t/,$line;
	#cytoplasmic genome for evaluating conversion error
	if (($arr[0] eq "Lambda_NEB") or ($arr[0] eq "ChrC") or ($arr[0] eq "ChrM")){ 
	if($arr[0] eq "Lambda_NEB"){$mC_sequenced_in_chrC1+=$arr[3];$C_sequenced_in_chrC1=$C_sequenced_in_chrC1+$arr[3]+$arr[4];}
...
print O $ARGV[0],"\t","Total reads","\t",$total_reads,"\t",
	...
	"mC level","\t",$mC/($mC+$unmC),"\t",
	#error
"error rate lambda:","	",$mC_sequenced_in_chrC1/$C_sequenced_in_chrC1,"\t",
"error rate ChrC:","	",$mC_sequenced_in_chrC2/$C_sequenced_in_chrC2,"\t",
"error rate ChrM:","	",$mC_sequenced_in_chrC3/$C_sequenced_in_chrC3,"\n";

close O;
$folder="$ARGV[0]_qc_map_statistic_report";
system("mkdir $folder");
system("mv $ARGV[0].* $folder");
system("mv $ARGV[0].R1.clean*t $folder");
